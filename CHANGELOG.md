...
- **Operational endpoints**: `/health`, `/api/ready`, admin `/api/admin/health`
- **Tech stack**: React 18 + Vite + Tailwind frontend, Node 20+ + Express 4 + Prisma ORM + Pino backend
- **Storage**: SQLite default (zero-config dev) / PostgreSQL recommended (production)
- **Optional Redis**: sessions, BaZi cache, OAuth state, password-reset tokens; falls back to in-memory when not configured
